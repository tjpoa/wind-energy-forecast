...
| Operational Read-only Copilot | Typed deterministic query layer, persistent local-only read-only API, versioned offline evaluation harness, optional default-disabled PostgreSQL query projection, sanitized local operational observability, provider-neutral single-tool Copilot core/offline runner, and offline injected-candidate evaluation/receipt boundary implemented; no provider or candidate evaluated | `POST /api/v1/operational-query`, observability endpoints, `wind_forecast.operational_query`, `wind_forecast.operational_copilot`, `wind_forecast.operational_copilot_models`, `wind_forecast.operational_observability`, `wind_forecast.operational_projection_reader`, `wind_forecast.operational_evaluation`, `wind_forecast.operational_candidate_evaluation`, 88-case synthetic dataset, dedicated tests, `docs/OPERATIONAL_COPILOT.md`, `docs/OPERATIONAL_COPILOT_CANDIDATE_EVALUATION.md` | Files/loaders remain authoritative and PostgreSQL is never cited. The Copilot accepts only `operational_query`, makes at most one selector/tool call, preserves `OperationalAnswer`, and refuses degraded observability; provider/model selection, egress, candidate evaluation, MCP, RAG, production authentication, remote exposure, and cloud design remain future work. |
...
5. Use the accepted offline evaluation harness and the
   ...
   candidate. The provider-neutral core and injected-candidate runner are
   implemented, but no provider or candidate has been evaluated; MCP, RAG,
   production authentication, remote exposure, and cloud stages remain out
   of scope.
