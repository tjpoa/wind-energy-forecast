...
| Operational Read-only Copilot | Typed deterministic query layer, local-only read-only API, and versioned offline evaluation harness implemented; PostgreSQL projection contract accepted; no Copilot evaluated | `POST /api/v1/operational-query`, `wind_forecast.operational_query`, `wind_forecast.operational_evaluation`, 88-case synthetic dataset, dedicated tests, `docs/OPERATIONAL_COPILOT.md`, `docs/OPERATIONAL_POSTGRES_PROJECTION.md` | PostgreSQL schema, migrations, projector, benchmark, and query integration are not implemented. Copilot, MCP, RAG, observability, production authentication, remote exposure, and cloud design remain future work. |
...
5. Deliver the accepted PostgreSQL operational-projection contract only through
   its four separately approved implementation plans: foundation/migrations,
   projector, benchmark, and query integration after a benchmark `GO`. No
   PostgreSQL runtime work is implemented.
6. Use the accepted offline evaluation harness as a mandatory gate for any
   separately approved future Copilot candidate. No candidate has been
   evaluated; Copilot, MCP, RAG, observability, production authentication,
   remote exposure, and cloud stages are not implemented.
7. Define and review an interactive-prediction UI/API contract separately,
