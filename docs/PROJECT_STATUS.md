...
5. Use the accepted offline evaluation harness as a mandatory gate for any
   separately approved future Copilot candidate. The provider-neutral core is
   implemented, but no provider or candidate has been evaluated; MCP, RAG,
   production authentication, remote exposure, and cloud stages remain out of
   scope.
