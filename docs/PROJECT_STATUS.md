...
5. Define the planned Operational Read-only Copilot contract/ADR, followed by
   a typed read-only query layer and evaluation harness in separate PRs. The
   Copilot, MCP, RAG, relational projection, and cloud stages are not
   implemented.
