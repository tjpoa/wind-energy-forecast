...
| Operational Read-only Copilot | Typed deterministic query layer, persistent local-only read-only API, versioned offline evaluation harness, optional default-disabled PostgreSQL query projection, sanitized local observability, provider-neutral single-tool core/offline runner, offline candidate boundary, and fixed OpenAI candidate adapter implemented; no live candidate evaluation or receipt | `POST /api/v1/operational-query`, observability endpoints, `wind_forecast.operational_query`, `wind_forecast.operational_copilot`, `wind_forecast.operational_observability`, `wind_forecast.operational_evaluation`, `wind_forecast.operational_candidate_evaluation`, `wind_forecast.operational_openai_candidate`, 88-case synthetic dataset, dedicated tests, `docs/OPERATIONAL_COPILOT.md`, `docs/OPERATIONAL_COPILOT_CANDIDATE_EVALUATION.md` | Files/loaders remain authoritative and PostgreSQL is never cited. The selected `gpt-5.4-mini-2026-03-17` adapter permits only sealed synthetic egress to the fixed Responses API with `store=false`, one call per case and zero retries. No API key was available, so no live calls, passed receipt, activation, MCP, RAG, production authentication, remote application exposure, or cloud design exists. |
...
5. Use the accepted offline evaluation harness and the
   [`OPERATIONAL_COPILOT_CANDIDATE_EVALUATION.md`](OPERATIONAL_COPILOT_CANDIDATE_EVALUATION.md)
   boundary as mandatory gates for any separately approved future Copilot
   candidate. The provider-neutral core, injected-candidate runner, and fixed
   OpenAI candidate adapter are implemented. A live 88-case run and passed
   receipt remain pending an operator-provided API key; MCP, RAG, production
   authentication, remote application exposure, and cloud stages remain out
   of scope.
